...
socket_t::socket_t() {
    ...
    if (master_mode) {
        ...
        std::cout << "Started master mode." << std::endl;
    }
    else {
        ...
        std::cout << "Started client mode." << std::endl;
    }
}
...
bool socket_t::get_msg(net_msg* ret) {
    std::vector<char> msg_raw;
    char buf;
    int got_bytes;

    // Получаем сообщение
    do {
        got_bytes = recv(s, &buf, sizeof(buf), 0);
        msg_raw.push_back(buf);
    } while (got_bytes == sizeof(buf));
    if (got_bytes < 1) return false;

    ret->type = (msg_types)msg_raw[0];
    size_t len = msg_raw.size() - 1;
    ret->len = len;

    // Копируем сообщение
    if (len > 1) {
        ret->data = new char[len];
        for (size_t i = 1; i < len; i++) ret->data[i - 1] = msg_raw[i];
    }
    return true;
}
